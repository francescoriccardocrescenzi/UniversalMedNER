...
if run_step 1; then
  ...
  python src/test_pipeline.py \
    ...
    --test_size "$TEST_SIZE" \
    --sfner_test_batch_size "$SFNER_TEST_BATCH_SIZE" \
    --sfner_grpo_max_completion_length "$SFNER_GRPO_MAX_COMPLETION_LENGTH" \
    --sfner_max_test_samples "$SFNER_MAX_TEST_SAMPLES" \
...
if run_step 2; then
  ...
  python src/train_pipeline.py \
    ...
    --sfner_sft_learning_rate "$SFNER_SFT_LEARNING_RATE" \
    --sfner_sft_lora_rank "$SFNER_SFT_LORA_RANK" \
    --sfner_sft_batch_size "$SFNER_SFT_BATCH_SIZE" \
    --sfner_sft_gradient_accumulation_steps "$SFNER_SFT_GRADIENT_ACCUMULATION_STEPS" \
    --sfner_sft_num_epochs "$SFNER_SFT_NUM_EPOCHS" \
    --sfner_sft_save_steps "$SFNER_SFT_SAVE_STEPS" \
    --sfner_sft_eval_steps "$SFNER_SFT_EVAL_STEPS" \
    --sfner_sft_max_steps "$SFNER_SFT_MAX_STEPS" \
    --sfner_grpo_learning_rate "$SFNER_GRPO_LEARNING_RATE" \
    --sfner_grpo_lora_rank "$SFNER_GRPO_LORA_RANK" \
    --sfner_grpo_batch_size "$SFNER_GRPO_BATCH_SIZE" \
    --sfner_grpo_gradient_accumulation_steps "$SFNER_GRPO_GRADIENT_ACCUMULATION_STEPS" \
    --sfner_grpo_num_generations "$SFNER_GRPO_NUM_GENERATIONS" \
    --sfner_grpo_max_completion_length "$SFNER_GRPO_MAX_COMPLETION_LENGTH" \
    --sfner_grpo_num_epochs "$SFNER_GRPO_NUM_EPOCHS" \
    --sfner_grpo_beta "$SFNER_GRPO_BETA" \
    --sfner_grpo_temperature "$SFNER_GRPO_TEMPERATURE" \
    --sfner_grpo_eval_steps "$SFNER_GRPO_EVAL_STEPS" \
    --sfner_grpo_max_steps "$SFNER_GRPO_MAX_STEPS" \
    --max_raw_samples "$MAX_RAW_SAMPLES" \
    --sfner_sft_max_train_samples "$SFNER_SFT_MAX_TRAIN_SAMPLES" \
    --sfner_sft_max_validation_samples "$SFNER_SFT_MAX_VALIDATION_SAMPLES" \
    --sfner_grpo_max_train_samples "$SFNER_GRPO_MAX_TRAIN_SAMPLES" \
    --sfner_grpo_max_validation_samples "$SFNER_GRPO_MAX_VALIDATION_SAMPLES"
fi
