...
def test_model_on_batch(model, processor, pile_ds, split="train", indices=list(range(8)), max_new_tokens=200, temperature=None, top_p=None):
...
def evaluate_dataset(
    ...
    which F1 mode(s) to compute ("soft", "strict", or both). `max_new_tokens` caps
...
def compute_structured_reward(gt_json, pred_json, bonus=STRUCTURED_REWARD_BONUS):
    """Compute the structured reward for a single sample (sum of its level components)."""
    return sum(compute_structured_reward_components(gt_json, pred_json, bonus).values())
...
def compute_sample_rewards(gt_json, pred_json, bonus=STRUCTURED_REWARD_BONUS):
    ...
    i.e. what `structured_reward_fn`'s summed outputs give a sample), and the
